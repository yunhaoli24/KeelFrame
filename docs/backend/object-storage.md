...
- 前端只访问后端返回的文件 path URL，由后端 `GET` 接口完成登录态、RBAC 与文件归属校验后转发到对象存储；S3/RustFS endpoint 与访问密钥仅后端内部使用。
- 用户上传文件通过登录 token 调用后端上传接口；后端维护 `s3_file` 文件记录并绑定上传用户。
...
- S3 文件记录模型：`backend/backend/app/s3/model/file.py`
- 本地 RustFS Docker 服务：`docker-compose.yml`

## 约束

- 新增上传能力时，优先复用默认对象存储配置，不再引入本地静态目录落盘链路。
- 普通用户上传接口不接收对象存储 endpoint、access key 或 secret key，也不接收存储配置 ID。
