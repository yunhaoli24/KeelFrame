...
这是 RBAC 全栈模板的后端服务，负责认证鉴权、权限模型、管理后台 API、任务调度、插件能力和后端基础设施。
...
- 后端 lint：在 `backend/` 目录运行 `bash backend/scripts/lint.sh`
- 后端测试：以 `backend/backend/app/admin/tests/` 下的 pytest 用例为入口
