...
- `docs/frontend/README.md`：前端文档索引（先看）

## 开发约定（必须）

- **Lint 错误**: 用户提到 lint 错误时，**必须运行** `cd backend && bash pre-commit.sh` 查看具体错误
- 提交前质量门禁：**必须先使用 subagent 执行 `code-simplifier` 和 `development-quality-check`，完成代码质量检查后再提交或发起 PR/MR。(不要给subagent太多上下文和假设，让他自己发现问题)**
- 不需要向后兼容：涉及旧功能更改时允许直接重构。
- **当前仓库现状（必须按现状开发）**：
  - 全栈开发统一通过根目录 `./dev.sh` 启动，脚本负责拉起所需服务
  - 禁止在前台直接启动服务；需要启动或重启本地全栈服务时，必须进入 `tmux full-app` 会话执行
  - 后端数据库 schema 与基线数据统一通过 Alembic migration 管理，启动脚本会先执行迁移再拉起服务
...
- 代码变更影响现状时，必须同步更新对应 `docs/` 文档索引。
- 新增"经验/踩坑/接口细节"时：优先写进 `docs/` 的对应文档，并在对应的"文档入口"里加链接。
